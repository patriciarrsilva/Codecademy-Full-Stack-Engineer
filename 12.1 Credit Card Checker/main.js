...
const convertToNewDigits = digits => {
...
const validateCred = digits => {
  const reversedDigits = reverseDigits(digits);

  const newDigits = convertToNewDigits(reversedDigits);

  const sum = sumDigits(newDigits);

  return !(sum % 10);
};

const findInvalidCards = creditCardNumbers =>
  creditCardNumbers.filter(
    creditCardNumber => !validateCred(creditCardNumber)
  );
...
const newMystery1 = '6396336092150117'; // invalid
const newMystery2 = '36693545906356'; // valid

const newBatch = [newMystery1, newMystery2];
...
const newCovertedBatch = newBatch.map(convertToArray);

const convertToValid = invalidNumbers => {
  return invalidNumbers.map(invalidNumber => {
    const reversedDigits = reverseDigits(invalidNumber);
    ...
    const dif = reversedDigits[0] - modulo;

    if (dif >= 0) {
      reversedDigits[0] -= modulo;
    } else {
      const difToModulo = 10 - modulo;

      reversedDigits[0] += difToModulo;
    }
...
const invalidNumbers = findInvalidCards(newCovertedBatch);
